...
#setwd("code/CarnivoreMonitoring-master") 
...
##   3.) Make sure R scripts are in working directory
  stopifnot(file.exists(c('./grid.txt',
                          './UniformLandscape.tif',
                          './createGrid.R',
                          './buildUseLayers.R',
                          './sample_ind.R',
                          './use_surface.R')))
##   4.) Set scenario number to run (example values provided)
  
  scenarios_to_test=c(1,2)  # Scenarios for this code example are defined on line 45
  RevGrid=F              # RevGrid=T to run 'reverse grids' where fisher are sampled on the marten sized grid and vice versa
  nRuns=250                # Change nRuns to set the # of replicates of the simulation.  
  
  # Note for large landscapes or population sizes run times per replicate can be long.  Recommend testing with a small
  # nRuns initially to test functionality and parameter settings before running large # of nRuns
...
source('./createGrid.R')
source('./buildUseLayers.R')
source('./sample_ind.R')
source('./use_surface.R')

# Landscape ####################################################################
MAP<-raster('./UniformLandscape.tif') 

# Grid #########################################################################
  grd<-read.table('./grid.txt', header=T)
...
#### Scenarios to simulate  ###################################
  ## N=initial population size, lmda=population growth rate (decline size), ESA=effective sampling area sizes
  
Scenarios<-expand.grid(N=c(150,250,400), lmda=c(0.933,0.978), ESA=c(25,6.25,1.56,0.39))

  output_dir<-paste0('./',sc) 
...
SPP<-list(list('Marten',1), list('Marten',2), list('Fisher',1), list('Fisher',2))
  ...
  # RevGrid   
   SPP<-list(list('Fisher',1),list('Fisher',2)) 
    if(sc==2)
